...
describe('[ stream-test / readable ]', function () {
  ...
  describe('[ data consumer ]', () => {
    ...
    xdescribe('[ EAGER-ASYNC-PRODUCER ]', () => {
      readableTest(makeStrings(8),
        (data) => readable({ eager: true, delayMs: 20 })({ encoding: 'utf8' })(data),
        dataConsumer,
        expectSameCallCount)
    })
  })
  ...
  describe('[ eager-readable consumer ]', function () {
    ...
    readableTest(makeStrings(8),
      (data) => readable({ eager: true })({ encoding: 'utf8', highWaterMark: 64 })(data),
      (stream, spy) => readableConsumer(stream, spy, { eager: true }),
      expectSameData)
  })
  ...
  describe('[ lazy-readable consumer ]', () => {
    xdescribe('[ EAGER-SYNC-PRODUCER ]', () => {
      readableTest(makeStrings(8),
        (data) => readable({ eager: true })({ encoding: 'utf8' })(data),
        (stream, spy) => readableConsumer(stream, spy, {}),
        expectSameData)
    })
  })
  ...
  describe('[ lazy-async-readable consumer ]', () => {
    xdescribe('[ EAGER-SYNC-PRODUCER ]', () => {
      readableTest(makeStrings(8),
        (data) => readable({ eager: true })({ encoding: 'utf8' })(data),
        (stream, spy) => readableConsumer(stream, spy, { delayMs: 0 }),
        expectSameData)
    })
  })
})
